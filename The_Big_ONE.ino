...
void setup(void) {
  ...
  new_s.initalizing("Connect to Grbl");
  while (Serial1.available() > 0) {
    String s = Serial1.readString();
    s.trim();
    if (contains(s, "Grbl 0.9j ['$' for help]") && contains(s, "['$H'|'$X' to unlock]")) {
      Serial.println("Grbl Succeeded");
      new_s.initalizing("Connection Succeeded");
      grbl_connect = true;
    } else {
      Serial.println("Grbl failed");
      new_s.initalizing("Connection Failed");
      grbl_connect = false;
    }
  }
  digitalWrite(EN , LOW);
...
void shield() {
  key_pad.getData();
  if (key_pad.default_key == "K" && jog_lock) {
    //unlock grbl
    grbl.unlock();
    delay(20);
    while (Serial1.available() > 0) {
      String s = Serial1.readString();
      Serial.println(s);
      if (contains(s.c_str() , "Unlocked")) {
        new_s.throwWarning("M/c Unlocked");
        jog_lock = false;
      } else {
        new_s.throwError("Can't Unlock");
      }
    }
    ...
    while (Serial1.available() > 0) {
      ...
      Serial.println(s);
      if (contains(s, "Hold")) {
        new_s.throwWarning("M/c Holeded");
      } else {
        new_s.throwError("Can't Hold");
      }
    }
    ...
    while (Serial1.available() > 0) {
      ...
      Serial.println(s);
      if (contains(s, "Grbl 0.9j ['$' for help]") && !contains(s, "['$H'|'$X' to unlock]")) {
        new_s.throwWarning("M/c Aborted");
        jog_lock = true;
      } else {
        new_s.throwError("Can't Abort");
      }
    }
    ...
    while (Serial1.available() > 0) {
      ...
      Serial.println(s);
      if (contains(s, "ok")) {
        new_s.throwWarning("M/c Resumed");
      } else {
        new_s.throwError("Can't Resume");
      }
    }
  }
}
...
bool contains(String big , String small) {
  char * pch;
  pch = strstr (big.c_str(), small.c_str());
  if (pch != NULL) {
    return true;
  } else {
    return false;
...
//    } else if (contains(s, "Grbl 0.9j ['$' for help]") && !contains(s, "['$H'|'$X' to unlock]")) {
